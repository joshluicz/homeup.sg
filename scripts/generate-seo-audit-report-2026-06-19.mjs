/**
 * Full SEO/GEO/AEO audit report — homeup.sg — 19 June 2026
 * Run: node scripts/generate-seo-audit-report-2026-06-19.mjs
 */
...
const DOMAIN = "homeup.sg";
const AUDIT_DATE = "19 June 2026";
const AUDIT_TYPE = "FULL AUDIT";
const SCORES = { seo: 8, geo: 8, aeo: 8 };
const COMBINED = SCORES.seo + SCORES.geo + SCORES.aeo;
const BASELINE = { seo: 7, geo: 8, aeo: 8, combined: 23, label: "14 June 2026 (lp.homeup.sg)" };
...
function makeMainSection() {
          ...
          children: [
            new TableCell({
              ...
              children: [
                para(
                  run(
                    "homeup.sg is now live on the apex domain with a mature SEO foundation. Since the 14 June lp.homeup.sg audit (+7 combined points from June baseline), the sitemap now lists 145 URLs (was 4 live), robots.txt no longer serves Crawl-delay, and canonicals point to https://homeup.sg. Page-level SEO remains strong: JSON-LD (Organization, WebSite, FAQPage, HowTo, SpeakableSpecification), /about NAP data, GST-aware pricing tables, and 125+ listing pages. Top gaps: consolidate homeup.com.sg with 301 redirects, fix client-only listing counts/stats for crawlers, enrich listing-detail SSR HTML, and submit the new sitemap in Google Search Console.",
                    { size: 11 },
                  ),
                ),
              ],
  ...
  const scoreRows = [
    [
      " SEO",
      SCORES.seo,
      "Apex domain live; 145-URL sitemap and clean robots.txt. Listing SSR and stat counters still client-dependent.",
    ],
    [
      " GEO",
      SCORES.geo,
      "Named CEA agents, office NAP, Haus Plus UEN, fixed-fee facts — highly citable for AI engines.",
    ],
    [
      " AEO",
      SCORES.aeo,
      "FAQ/HowTo/Speakable markup intact; Playbook hub still thin ('Content coming soon').",
    ],
  ];
      ...
      rows: [
        ...
        new TableRow({
          children: [
            ...
            dCell(`Up from ${BASELINE.combined}/30 on ${BASELINE.label}`, C.lgray),
          ],
        }),
      ],
    }),
  ...
  const pageData = [
    ["homeup.sg/", "Homepage", "JSON-LD stack; FAQ; hero stats animate from 0 (crawler sees 0 until JS)"],
    ["homeup.sg/about", "About / E-E-A-T", "NAP, C&H + Haus Plus entity, 6 named agents with CEA numbers"],
    ["homeup.sg/sell", "Sell hub", "Pricing, HowTo/FAQ schema, GST comparison table"],
    ["homeup.sg/sell-hdb", "Sell HDB", "Type-specific FAQs and process timeline"],
    ["homeup.sg/buy", "Buy hub", "Packages, calculator, FAQ section"],
    ["homeup.sg/buy-hdb", "Buy HDB", "Grant/loan FAQs, $1,999 fee messaging"],
    ["homeup.sg/listings", "Listings index", "125 active listings; filters; ItemList schema"],
    ["homeup.sg/listings/amo-residence", "Listing detail", "Metadata + JSON-LD present; body loaded client-side only"],
    ["homeup.sg/agents/dennis-lim", "Agent profile", "CEA R055990G, social links, YouTube embed"],
    ["homeup.sg/playbook", "Playbook hub", "3 topic cards marked 'Content coming soon'"],
    ["homeup.sg/privacy-policy", "Legal / PDPA", "DPO email, analytics disclosure"],
    ["homeup.sg/robots.txt", "Crawl config", "Allow all; Sitemap: https://homeup.sg/sitemap.xml — no Crawl-delay"],
    ["homeup.sg/sitemap.xml", "Sitemap", "145 URLs — hubs, 125+ listings, agents, sub-landings"],
  ];
  children.push(
    new Table({
      width: { size: 100, type: WidthType.PERCENTAGE },
  ...
  children.push(
    findingTable([
      ...
      [
        "Canonical tags",
        "Self-referencing canonical on https://homeup.sg/ — apex domain migration complete.",
        "Good",
      ],
      [
        "Open Graph / Twitter",
      ...
      [
        "Sitemap.xml",
        "Live file lists 145 URLs including /sell, /about, all agents, and 125+ listing pages. lastmod June 2026.",
        "Good",
      ],
      [
        "robots.txt",
        "Allow: / ; Sitemap at homeup.sg. Crawl-delay removed (fixed since 14 June audit).",
        "Good",
      ],
      [
        "Apex domain",
        "Site now on homeup.sg (was lp.homeup.sg). Brand authority consolidates on primary domain.",
        "Good",
      ],
      [
        "Client-rendered stats",
        "Homepage hero counters and ListingCount components start at 0 or em-dash until JS runs — weak for crawlers.",
        "Needs Attention",
      ],
      [
        "Listing detail SSR",
        "/listings/[slug] metadata and JSON-LD are server-rendered but page body fetches client-side only.",
        "Needs Attention",
      ],
      [
        "homeup.com.sg",
        "Secondary domain should 301 to homeup.sg to avoid split authority.",
        "Needs Attention",
      ],
    ]),
  );
  ...
  const recs = [
    [
      "Critical",
      "301 redirect homeup.com.sg and www.homeup.com.sg to https://homeup.sg",
      "SEO",
      "Low",
      "Very High",
    ],
    [
      "Critical",
      "Submit https://homeup.sg/sitemap.xml in Google Search Console (new apex domain)",
      "SEO",
      "Low",
      "Very High",
    ],
    [
      "High",
      "Server-render listing counts and listing detail HTML (not client-only fetch)",
      "SEO / GEO",
      "Medium",
      "High",
    ],
    [
      "High",
      "Add noscript/static fallbacks for homepage transaction stats (1000+, 860+ HDB)",
      "GEO",
      "Low",
      "Medium",
    ],
    [
      "Medium",
      "Hide or exclude /playbook from sitemap until content is published",
      "SEO",
      "Low",
      "Medium",
    ],
    ...
    [
      "Quick Win",
      "Verify Vercel Analytics + GA4 both tracking on production homeup.sg",
      "SEO",
      "Low",
      "Low",
    ],
  ...
  const strengths = [
    [
      "Apex domain migration",
      `Combined ${COMBINED}/30 vs ${BASELINE.combined}/30 on ${BASELINE.label}. homeup.sg is now canonical.`,
    ],
    [
      "Full sitemap deployed",
      "145 URLs live including 125+ listing pages, all sell/buy hubs, agents, and about.",
    ],
    [
    ...
    [
      "125+ live listings",
      "Long-tail local property URLs indexed via comprehensive sitemap.",
    ],
  ];
...
const docxPath = path.join(ROOT, "seo-audit-homeup-sg-2026-06-19.docx");
...
for (const bin of sofficeCandidates) {
  const result = spawnSync(bin, ["--headless", "--convert-to", "pdf", docxPath, "--outdir", ROOT], {
    stdio: "pipe",
    encoding: "utf8",
  });
...
if (!converted) {
  console.warn("PDF conversion skipped — install LibreOffice or set SOFFICE_PATH.");
}
